        ...
        initTelegram() {
            if (window.Telegram && window.Telegram.WebApp) {
                ...
                if (initData && initData.user) {
                    ...
                    // Альтернативный способ получения chat_id
        ...
        async loadDrawing() {
            ...
            try {
                const response = await fetch(
                    `/api/drawing/${this.chatId}/${this.selectedYear}/${this.selectedMonth + 1}`
                );
                ...
                if (data.drawing_data) {
                    ...
                    // Очищаем canvas если нет сохраненного рисунка
                    if (this.ctx && this.canvas) {
                        this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
                    }
                }
            } catch (error) {
                console.error('Ошибка загрузки рисунка:', error);
            }
        },
        restoreDrawing() {
